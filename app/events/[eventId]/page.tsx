...
export default async function EventDetailPage({ params }: Props) {
  ...
  const now = new Date()
  const registrationOpensDate = event.registration_opens_at ? new Date(event.registration_opens_at) : null
  const deadlineDate = event.registration_deadline ? new Date(event.registration_deadline) : null
  const startDate = event.start_at ? new Date(event.start_at) : null
  const targetDate = registrationOpensDate && registrationOpensDate > now
