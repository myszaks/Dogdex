...
export async function POST() {
  ...
  const canReconcileBusiness = Boolean(businessAccess?.can('refunds.manage'))
  if (!isPayoutRole(role) && !canReconcileBusiness) return NextResponse.json({ error: 'Brak uprawnień' }, { status: 403 })
  if (!hasServiceRoleKey()) return NextResponse.json({ error: 'Brak konfiguracji serwera' }, { status: 503 })
  try {
    return NextResponse.json(canReconcileBusiness
      ? await reconcileEventPayments(undefined, businessAccess!.profile.id)
      : await reconcileEventPayments(user.id))
  } catch (error) {
