...
export default function EventCard({ event, registeredCount, extraActions, hidePublicActions }: EventCardProps) {
  ...
  const registrationNotStarted = Boolean(
    ...
    && new Date(event.registration_opens_at).getTime() > renderedAt
    && dispStatus === 'upcoming'
  )
    ...
    <div className="bg-card rounded-3xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col group">
      ...
      <div className="relative w-full aspect-video shrink-0 overflow-hidden">
        ...
        {event.last_significant_change &&
          ...
          renderedAt - new Date(event.last_significant_change).getTime() < 7 * 24 * 60 * 60 * 1000 && (
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex-1">
          ...
          {event.created_by && (
            <Link href={`/organizers/${event.created_by}`} className="mb-2 flex items-center gap-1 text-xs text-muted-foreground hover:text-accent">
          ...
          <div className="flex flex-col gap-1">
            ...
            {(event.registration_deadline || registrationNotStarted) && dispStatus === 'upcoming' && (
              <p className={cn('text-xs flex items-center gap-1.5', regOpen ? 'text-muted-foreground' : 'text-orange-600 font-medium')}>
                ...
                {registrationNotStarted && event.registration_opens_at
                  ...
                  : regOpen && event.registration_deadline
                    ? `Zapisy do ${formatDate(event.registration_deadline)}`
                    : 'Zapisy zamknięte'}
              </p>
            )}
          </div>
